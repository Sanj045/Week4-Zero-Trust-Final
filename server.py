...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    write_audit_log(form_data.username, "Đăng nhập hệ thống", "200 OK")
    
    access_token = create_access_token(data={"sub": form_data.username, "role": user_row[1]})
...
async def get_audit_logs(current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "admin":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Cấm truy cập!")
    
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    # Lấy 50 log mới nhất, xếp từ mới đến cũ
...
async def get_all_users(current_user: dict = Depends(get_current_user)):
    ...
    # 2. Lấy toàn bộ danh sách từ Database
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("SELECT username, role FROM users")
    
    # Gom dữ liệu thành một danh sách (list)
    users_list = []
    for row in cursor.fetchall():
        users_list.append({"username": row[0], "role": row[1]})
...
async def delete_user(target_user: str, current_user: dict = Depends(get_current_user)):
    ...
    # 2. Hardcode bảo vệ: Không cho phép ai xóa tài khoản Root Admin
    if target_user == "admin":
        raise HTTPException(status_code=400, detail="Không thể xóa tài khoản Root Admin!")

    # 3. Thực hiện lệnh xóa trong Database SQLite
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
